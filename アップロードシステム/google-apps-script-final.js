/**
 * Google Apps Script for Advalay Store Registration - 最終版
 * 
 * フォームID: 1phMxnc_fvzDNmx-HJwEW2vOGHVVnaj6Q3jBnpqq08JU
 * スプレッドシートID: 1tJIC3abg2CDZUipCBDL65rA2hwTNWKQJBaOZLtrOdJs
 * 
 * セットアップ手順:
...
function recordResult(formData, result, success) {
  try {
    ...
    if (success) {
      sheet.getRange(lastRow, resultColumn).setValue(`✅ 成功 - ${result.data.storeToken}`);
    } else {
      sheet.getRange(lastRow, resultColumn).setValue(`❌ 失敗 - ${result.error}`);
    }
    
    console.log('📊 結果をスプレッドシートに記録:', success ? '成功' : '失敗');
  ...
  try {
    ...
    if (lastRow > 1) {
      ...
      if (lastRow > 1) {
        ...
        console.log('📝 最新の回答:', lastResponse);
      }
    }
