...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/admin/upload-stats/stores?${queryParams.toString()}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: errorData.message || 'Failed to fetch store upload stats' }, { status: response.status });
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch store upload stats' },
      { status: 500 }
    );
  }
}
