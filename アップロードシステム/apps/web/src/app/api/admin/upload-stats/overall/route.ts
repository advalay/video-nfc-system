...
export async function GET(request: NextRequest) {
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/admin/upload-stats/overall?${queryParams.toString()}`, {
      headers: {
    ...
    if (!response.ok) {
      ...
      return NextResponse.json({ error: errorData.message || 'Failed to fetch overall upload stats' }, { status: response.status });
    }
    ...
    return NextResponse.json(
      { error: 'Failed to fetch overall upload stats' },
      { status: 500 }
    );
  }
}
