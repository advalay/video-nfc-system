...
### 問題の詳細
1. **組織管理ページ**: 全組織で販売店数・動画数・容量が全て0
2. **システム統計ページ**: Mock dataが表示されている（実DBとの不一致）

### 原因分析
- テストアカウント作成時に組織データは登録済み
- しかし、販売店データ、動画データが不足
- APIがMock dataを返している可能性

## 📋 追加タスク: DBデータ整備（本番準備）

### 13. DBデータ整備スクリプト作成と実行
- [ ] DynamoDB内の既存データ確認
- [ ] 販売店データ登録スクリプト作成
  - パートナー企業Aに紐づく販売店A1の登録
  - 組織-販売店の関連付け確認
- [ ] テスト用動画メタデータ登録（オプション）
  - 各販売店に最低1つの動画データ
  - S3バケットとの整合性確認
- [ ] Cognito User - DynamoDB組織データの紐付け確認
  - Custom attributes (custom:organizationId, custom:shopId)
- [ ] データ整合性テスト
  - 組織管理ページで正しいデータ表示確認
  - システム統計ページでMock dataではなく実データ表示確認

### 14. Mock data削除
- [ ] システム統計APIのMock data返却ロジック確認
- [ ] 組織管理APIのMock data返却ロジック確認
- [ ] 実DBからデータ取得するように修正
...
3. 🔴 **DBデータ整備**（新規・最優先）
   ...
   - Mock data削除
4. **全ユーザーロールでの動作確認**
5. Lambda関数とapi-client.tsの開発モード関連コード削除
6. エラーハンドリング確認
