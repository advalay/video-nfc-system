  ...
  const handleSubmit = async (e: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (process.env.NODE_ENV === 'development') {
        ...
        toast.success('ログインに成功しました！（開発モード）');
        setTimeout(() => {
          window.location.href = '/videos';
        }, 500);
        return;
      ...
      if (isSignedIn) {
        ...
        // ページをリロードしてからリダイレクト（認証状態を確実に更新）
        setTimeout(() => {
          window.location.href = '/videos';
        }, 500);
      } else if (nextStep.signInStep === 'CONFIRM_SIGN_IN_WITH_NEW_PASSWORD_REQUIRED') {
        toast.error('パスワードの変更が必要です');
        // パスワード変更画面に遷移する処理を追加可能
      } else {
        console.log('Unexpected nextStep:', nextStep);
        toast.error('追加の認証手順が必要です');
      }
    } catch (error: unknown) {
      ...
      if (error instanceof Error) {
        if (error.name === 'UserAlreadyAuthenticatedException') {
          errorMessage = '既にログインしています';
          // 既にログイン済みの場合は動画一覧ページにリダイレクト
          setTimeout(() => {
            window.location.href = '/videos';
          }, 1000);
        ...
        {process.env.NODE_ENV === 'development' && (
          <div className="mt-6 bg-yellow-50 border border-yellow-200 rounded-lg p-4">
            ...
            <div className="text-xs text-yellow-700 space-y-1">
              ...
              <p>• 代理店A管理者: org-a-admin@example.com</p>
              <p>• 東京支店: tokyo-shop@example.com</p>
              <p className="mt-2 font-medium">パスワードはREADME参照</p>
            </div>
          </div>
        )}
