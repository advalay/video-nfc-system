...
export default function UploadPage() {
  ...
  const validateFile = (file: File): boolean => {
    ...
    const isTypeValid = validTypes.some(type => file.type === type) ||
      validExtensions.some(ext => file.name.toLowerCase().endsWith(ext));

    if (!isTypeValid) {
      toast.error('MP4形式の動画またはMP3形式の音声ファイルのみアップロード可能です');
      return false;
    }
    ...
    <ProtectedRoute>
      <Layout>
        ...
        <div className="flex items-center space-x-4 mb-8">
          ...
          <span className="text-xl font-bold text-gray-900">コンテンツアップロード</span>
        </div>
              ...
              {!isUploading && (
                ...
                >
                  <input
                    ref={fileInputRef}
                    type="file"
                    accept="video/mp4,audio/mpeg,audio/mp3,.mp3"
