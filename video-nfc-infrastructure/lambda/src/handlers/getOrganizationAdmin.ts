...
export const handler = async (event: APIGatewayProxyEvent): Promise<APIGatewayProxyResult> => {
  try {
    ...
    for (const cognitoUser of listUsersResult.Users || []) {
      ...
      // グループをチェック
      const groups = getUserResult.UserAttributes?.find((attr: any) => attr.Name === 'cognito:groups')?.Value || '';
    ...
    const email = adminUser.Attributes?.find((attr: any) => attr.Name === 'email')?.Value || '';
